...
class EVEventData(BaseModel):
    """
    Payload for `ev/*` events. The event describes the change of an EV's state at a charging station.   # noqa: E501
    """
    ...
    evse_id: Optional[StrictStr] = Field(None, alias="evseID", description="Electirc Vehicle Supply Equipment ID")
    state: StrictStr = Field(..., description="State of the EV charging station.")
    __properties = ["applianceID", "gatewayID", "model", "manufacturer", "type", "kind", "name", "serialNumber", "systemID", "systemName", "evseID", "state"]
    ...
    def state_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('plugged'):
            raise ValueError("must be one of enum values ('plugged')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> EVEventData:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> EVEventData:
        """Create an instance of EVEventData from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return EVEventData.parse_obj(obj)

        _obj = EVEventData.parse_obj({
